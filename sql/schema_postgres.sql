...
-- `ogham init` and the test suite apply this the same way via
-- ogham.schema_apply.render_schema_sql (plain text substitution, not psql
-- variable interpolation, so it isn't affected by the dollar-quote gap).
...
--
-- search_path is pinned on both lifecycle trigger functions: without it they
-- resolve unqualified names against the CALLER's search_path, the standard
-- Postgres privilege-escalation route. Migration 020 pinned it on existing
-- installs and the schema files were never updated to match.
...
-- SECURITY DEFINER + pinned search_path: migration 036 hardened this on
-- existing installs and the schema files were never updated to match.
...
-- rationale.
--
-- 037 revokes ELEVEN functions from PUBLIC and does so unconditionally
-- ("PUBLIC always exists -- safe to REVOKE on every install"); only three of
-- them were ever backported here, so a fresh install left the other eight
...
-- lifecycle and graph RPCs they name are created.
...
-- 043_entity_aliases.sql for design notes. No RLS / grants here -- vanilla
-- Postgres has no auth.uid() and no anon role.
...
-- SUPERSEDES intentionally omitted per TBU-109 -- redundant with
-- entity_edges.valid_to / superseded_by.
...
-- ── Revoke PUBLIC EXECUTE on the SECURITY DEFINER RPCs (migration 037) ──
-- EXECUTE defaults to PUBLIC in Postgres, so a SECURITY DEFINER function is
-- callable by every role until something says otherwise. Migration 037 does
-- this unconditionally for existing installs; without it here, a fresh install
-- is strictly more permissive than an upgraded one. Placed after the RPCs
-- above because REVOKE needs the function to exist.
REVOKE EXECUTE ON FUNCTION lifecycle_advance_fresh_to_stable(text, timestamptz, double precision, double precision) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION lifecycle_close_editing_windows(text, timestamptz) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION lifecycle_open_editing_window(uuid[]) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION lifecycle_pipeline_counts(text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION hebbian_strengthen_edges(text[], text[], real, real) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION entity_graph_density(text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION suggest_unlinked_by_shared_entities(uuid, text, integer, integer) FROM PUBLIC;

-- 037 has a second, role-guarded half: where anon/authenticated exist it
-- narrows them too, undoing the broad grant the 035 block above makes. Vanilla
-- Postgres has neither role, so this is a no-op on the intended install path --
-- it is here so that a database which does have them ends up in the same state
-- whether it was installed fresh or migrated.
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'authenticated') THEN
